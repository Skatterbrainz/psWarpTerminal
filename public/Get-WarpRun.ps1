function Get-WarpRun {
    ...
    In ById mode, retrieve the conversation transcript for the run instead of run status.
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'ById') {
            $a = [System.Collections.Generic.List[string]]@('run', 'get', $TaskId)
            if ($Conversation.IsPresent) { $a.Add('--conversation') }
